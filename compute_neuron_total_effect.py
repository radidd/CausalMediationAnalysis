...
def main(folder_name, model_name, neg_test_file = 'neg_test.json'):
    ...
    for path in files:
        ...
        means_dict[ntype][orig_correctness].append(mean_total)
        means_dict["overall"][orig_correctness].append(mean_total)

    for ntype in means_dict.keys():
        for oc in means_dict[ntype].keys():
            print(
            "The total effect of this model for {} {} is {:.3f}".format(ntype, oc, (np.mean(means_dict[ntype][oc]) - 1))
    )
