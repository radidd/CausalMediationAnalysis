...
def main(folder_name):
    ...
    for ntype in ["v-action", "v-existential", "np-nonexistential", "np-existential", "np-num2none", "sw"]:
        ...
        plt.savefig(
            os.path.join(folder_name, ntype + "_neuron_layer_effect.pdf"),
            format="pdf",
            bbox_inches="tight",
        )
        ...
        for k, v in modelname_to_effects.items():
            ...
            plt.savefig(
                os.path.join(folder_name, ntype + "_neuron_layer_effect_" + k + ".pdf"),
                format="pdf",
                bbox_inches="tight",
            )
    ...
    for k, v in modelname_to_effects.items():
        ...
        plt.savefig(
            os.path.join(folder_name, k + "_neuron_layer_effect.pdf"),
            format="pdf",
            bbox_inches="tight",
        )
