...
def analyze_effect_results(results_df, effect, word, gold_label, savefig=None):
    # calculate odds.
    if gold_label == "True":
        odds_base = (
...
def get_all_effects(fname, gold_label_base):
    """
    ...
    analyze_effect_results(
        results_df=indirect_result_df, effect="indirect", word="all", gold_label=gold_label_base
    )
    fname = fname.replace("_indirect_", "_direct_")
    direct_result_df = pd.read_csv(fname)
    analyze_effect_results(
        results_df=direct_result_df, effect="direct", word="all", gold_label=gold_label_base
    )
...
def main(folder_name="results/20191114_neuron_intervention/", model_name="distilgpt2"):
    ...
    # TODO-RADI: load the gold labels + negation types

    labels_and_neg_types = {}
    with open('neg_test.json', 'r') as test_set:
        for line in test_set:
            js_line = json.loads(line)
            try:
                labels_and_neg_types[js_line["identifier"]] = (js_line["label"], js_line["negation_type"])
            except KeyError:
                labels_and_neg_types[js_line["identifier"]] = (js_line["label"], None)
    ...
    dfs = []
    for path in files:
        id_base = path.split("/")[-1].split("_")[0]
        #id_alt = path.split("/")[-1].split("_")[1]

        gold_label_base = labels_and_neg_types[id_base][0]
        dfs.append(get_all_effects(path, gold_label_base))
    df = pd.concat(dfs)
